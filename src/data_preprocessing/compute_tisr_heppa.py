...
"""Computes TOA incident solar radiation compatible with ERA5 from Total Solar irradiance (TSI) which is provided
as official forcing for CMIP6 (https://gmd.copernicus.org/articles/10/2247/2017/)
"""
import argparse
import logging
import os
...
def get_toa_incident_solar_radiation(
  ...
  tsi = tsi_data.interp(time=timestamps.strftime('%Y-%m-%dT%H').tolist()).values
...
def main(tsir_data_path, output_dir, path_to_reference_file, start_year, end_year, time_delta, start_month_day_hour):

    logging.info(f'Computing TOA incident solar radiation for years: {start_year} to {end_year}')

    reference_data = xa.open_dataset(path_to_reference_file)
    solar_data = xa.open_dataset(tsir_data_path).tsi

    lat = reference_data['lat'].values
    lon = reference_data['lon'].values
    ...
    for year in range(start_year, end_year+1):
        ...
        start_date = f'{year}-{start_month_day_hour}'
        timestamps = pd.date_range(start=start_date, end=f'{year}-12-31T23', freq=f'{time_delta}')

        logging.info(f'Timestamps: {timestamps[:5]} ... {timestamps[-5:]}')
        ...
        logging.info(f'Saving file: {os.path.join(output_dir, f"{year}.nc")}')
        ds.to_netcdf(os.path.join(output_dir, f'{year}.nc'))


if __name__ == "__main__":
  
    # Parse command line arguments
    parser = argparse.ArgumentParser(description='Compute TOA incident solar radiation')
    parser.add_argument('--tsir_data_path', type=str, help='Path to the solar data file')
    parser.add_argument('--output_dir', type=str, help='Path to the output directory')
    parser.add_argument('--path_to_reference_file', type=str, help='Path to the reference file for grid coordinates')
    parser.add_argument('--start_year', type=int, help='Start year for the computation',  default=1850)
    parser.add_argument('--end_year', type=int, help='End year for the computation',  default=1978)
    parser.add_argument('--time_delta', type=str, help='Time delta to compute tsir',  default='1h')
    parser.add_argument('--start_month_day_hour', type=str, help='Start yearly time series at. Format: month-dayThour',  default='01-01T00')
    args = parser.parse_args()

    args = vars(args)
    args['output_dir'] = os.path.join(os.environ['DATA_DIR'], args['output_dir'])
    args['path_to_reference_file'] = os.path.join(os.environ['DATA_DIR'], args['path_to_reference_file'])
    args['tsir_data_path'] = os.path.join(os.environ['DATA_DIR'], args['tsir_data_path'])
    ...
    logging.basicConfig(
        ...
        datefmt='%Y-%m-%d %H:%M:%S',
        handlers=[
            logging.FileHandler(os.path.join(args['output_dir'], f'logfile_{current_time}.log'), mode='w'),
            logging.StreamHandler()
        ]
    )

    logging.info(f'Command: {" ".join(os.sys.argv)}')

    shutil.copy(os.path.abspath(__file__), os.path.join(args['output_dir'], f'process_script_{current_time}.py'))
    logging.info(f"Copied {os.path.abspath(__file__)} to {os.path.join(args['output_dir'], 'process_script.py')}")
